...
public class Solution {
    public List<string> FindWords(char[][] board, string[] words) {
        ...
        //(though REFS based soln MIGHT be the best if it increases efficiency)
        ...
        return Attempt1.FindWordsNonHashSet(board, words);
    }
}
...
public static class Attempt1
{
    ...
    public static List<string> FindWordsNonHashSet(char[][] board, string[] words) {
        root = new TrieNode();
        maxWordLen = 0;
        ...
        List<string> res = new(); //IMPORTANT!!! I DID NOT THINK ABOUT THE SAME WORD APPEARING MULTIPLE TIMES!! (good clarification question?)
        List<char> wordSoFar = new(maxWordLen);
        ...
        for(int r=0; r<board.Length;r++)
        {
            for(int c=0; c<board[0].Length;c++)
            {
                dfsNonHashSet(board, r, c, root, wordSoFar, res);
            }
        }
    ...
    private static void dfsNonHashSet(char[][] board, int r, int c, TrieNode prevNode, List<char> wordSoFar, List<string> res)
    {
        // Console.WriteLine($"r:{r} / {board.Length-1}, c:{c} / {board[0].Length-1}");
        if(r<0||r>=board.Length||c<0||c>=board[0].Length||board[r][c]==visited||wordSoFar.Count==maxWordLen) //im dumb cuz I spend so long debugging just to realize I was using r > length and c > length instead of >= !!!
        {
        ...
        if(curNode.isWord)
        {
            res.Add(string.Concat(wordSoFar));
            curNode.isWord = false;
        }

        dfsNonHashSet(board, r-1, c, curNode, wordSoFar, res);    //I SPENT SOOO FUCKING LONG TO DEBUG THE IF CONDITIONS AT THE TOP THAT I DIDN'T REALIZE THE PROBLEM WAS I WAS PASSING ROOT INSTEAD OF CURRENT NODE BY MISTAKE WTFFF!!!
        dfsNonHashSet(board, r+1, c, curNode, wordSoFar, res);
        dfsNonHashSet(board, r, c-1, curNode, wordSoFar, res);
        dfsNonHashSet(board, r, c+1, curNode, wordSoFar, res);
